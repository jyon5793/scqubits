...
class ZeroPi(base.QubitBaseClass, serializers.Serializable, NoisyZeroPi):
   ...
   """
    ...
    def __init__(
        ...
        EC: Optional[bc.backend.float_],
        ng: bc.backend.float_,
        flux: bc.backend.float_,
        grid: Grid1d,
        ncut: bc.backend.int_,
        dEJ: bc.backend.float_ = 0.0,
        dCJ: bc.backend.float_ = 0.0,
        ...
        self._default_grid = discretization.Grid1d(-bc.backend.pi / 2, 3 * bc.backend.pi / 2, 200)
    ...
    def get_ECS(self) -> bc.backend.float_:
    ...
    def potential(self, phi: bc.backend.ndarray, theta: bc.backend.ndarray) -> bc.backend.ndarray:
        """
        Returns
        -------
        ...
        """
        return (
            -2.0 * self.EJ * bc.backend.cos(theta) * bc.backend.cos(phi - 2.0 * bc.backend.pi * self.flux / 2.0)
            + self.EL * phi**2
            + 2.0 * self.EJ
            + self.EJ
            * self.dEJ
            * bc.backend.sin(theta)
            * bc.backend.sin(phi - 2.0 * bc.backend.pi * self.flux / 2.0)
        )

    def sparse_kinetic_mat(self) -> bc.backend.csc_matrix:
        """
        ...
        identity_phi = bc.backend.spidentity(pt_count, format="csc")
        identity_theta = bc.backend.spidentity(dim_theta, format="csc")
        ...
        diag_elements = (
            2.0
            * self.ECS
            * bc.backend.square(bc.backend.arange(-self.ncut + self.ng, self.ncut + 1 + self.ng))
        )
        kinetic_matrix_theta = bc.backend.to_csc_matrix(bc.backend.dia_matrix(
            (diag_elements, [0]), shape=(dim_theta, dim_theta)
        ))
        kinetic_matrix = bc.backend.spkron(
            kinetic_matrix_phi, identity_theta, format="csc"
        ) + bc.backend.spkron(identity_phi, kinetic_matrix_theta, format="csc")
        if self.dCJ != 0:
            kinetic_matrix -= (
                2.0
                * self.ECS
    ...
    def sparse_potential_mat(self) -> bc.backend.csc_matrix:
        """
        ...
        phi_inductive_vals = self.EL * bc.backend.square(grid_linspace)
        phi_inductive_potential = bc.backend.to_csc_matrix(bc.backend.dia_matrix(
            (phi_inductive_vals, [0]), shape=(pt_count, pt_count)
        ))
        phi_cos_vals = bc.backend.cos(grid_linspace - 2.0 * bc.backend.pi * self.flux / 2.0)
        phi_cos_potential = bc.backend.to_csc_matrix(bc.backend.dia_matrix(
            (phi_cos_vals, [0]), shape=(pt_count, pt_count)
        ))
        phi_sin_vals = bc.backend.sin(grid_linspace - 2.0 * bc.backend.pi * self.flux / 2.0)
        phi_sin_potential = bc.backend.to_csc_matrix(bc.backend.dia_matrix(
            (phi_sin_vals, [0]), shape=(pt_count, pt_count)
        ))

        theta_cos_potential = bc.backend.to_csc_matrix((
            -self.EJ
            * (
                bc.backend.dia_matrix(
        ...
        potential_mat = (
            bc.backend.spkron(phi_cos_potential, theta_cos_potential, format="csc")
            + bc.backend.spkron(phi_inductive_potential, self._identity_theta(), format="csc")
            + 2
            * self.EJ
            * bc.backend.spkron(self._identity_phi(), self._identity_theta(), format="csc")
        )
        if self.dEJ != 0:
            potential_mat += (
                self.EJ
                * self.dEJ
                * bc.backend.spkron(phi_sin_potential, self._identity_theta(), format="csc")
    ...
    def sparse_d_potential_d_flux_mat(self) -> bc.backend.csc_matrix:
        r"""
        ...
        """
        op_1 = bc.backend.spkron(
            self._sin_phi_operator(x=-2.0 * bc.backend.pi * self.flux / 2.0),
            self._cos_theta_operator(),
            format="csc",
        )
        op_2 = bc.backend.spkron(
            self._cos_phi_operator(x=-2.0 * bc.backend.pi * self.flux / 2.0),
            self._sin_theta_operator(),
            format="csc",
        )
        return -2.0 * bc.backend.pi * self.EJ * op_1 - bc.backend.pi * self.EJ * self.dEJ * op_2
    ...
    def sparse_d_potential_d_EJ_mat(self) -> bc.backend.csc_matrix:
    ...
    def _identity_phi(self) -> bc.backend.csc_matrix:
        ...
        """
        pt_count = self.grid.pt_count
        return bc.backend.spidentity(pt_count, format="csc")
    ...
    def i_d_dphi_operator(self) -> bc.backend.csc_matrix:
        r"""
    ...
    def _phi_operator(self) -> bc.backend.dia_matrix:
        r"""
    ...
    def phi_operator(
        ...
        """
        native = bc.backend.spkron(self._phi_operator(), self._identity_theta(), format="csc")
        return self.process_op(native_op=native, energy_esys=energy_esys)
    ...
    def n_theta_operator(
        ...
        native = bc.backend.spkron(self._identity_phi(), n_theta_matrix, format="csc")
        return self.process_op(native_op=native, energy_esys=energy_esys)

    def _sin_phi_operator(self, x: bc.backend.float_ = 0) -> bc.backend.csc_matrix:
        r"""
        Operator :math:`\sin(\phi + x)`, acting only on the `\phi` Hilbert subspace.x
        """
        pt_count = self.grid.pt_count

        vals = bc.backend.sin(self.grid.make_linspace() + x)
        sin_phi_matrix = bc.backend.to_csc_matrix(bc.backend.dia_matrix(
            (vals, [0]), shape=(pt_count, pt_count)
        ))
        return sin_phi_matrix

    def _cos_phi_operator(self, x: bc.backend.float_ = 0) -> bc.backend.csc_matrix:
        r"""
        Operator :math:`\cos(\phi + x)`, acting only on the `\phi` Hilbert subspace.
        """
        pt_count = self.grid.pt_count

        vals = bc.backend.cos(self.grid.make_linspace() + x)
        cos_phi_matrix = bc.backend.to_csc_matrix(bc.backend.dia_matrix(
            (vals, [0]), shape=(pt_count, pt_count)
        ))
    ...
    def _cos_theta_operator(self) -> bc.backend.csc_matrix:
        ...
        """
        dim_theta = 2 * self.ncut + 1
        cos_theta_matrix = bc.backend.to_csc_matrix((
            0.5
            * (
                bc.backend.dia_matrix(
                    ([1.0] * dim_theta, [-1]), shape=(dim_theta, dim_theta)
    ...
    def _sin_theta_operator(self) -> bc.backend.csc_matrix:
        ...
        """
        dim_theta = 2 * self.ncut + 1
        sin_theta_matrix = bc.backend.to_csc_matrix((
            -0.5
            * 1j
            * (
                bc.backend.dia_matrix(
    ...
    def plot_potential(
        ...
        contour_vals: Union[List[bc.backend.float_], bc.backend.ndarray] = None,
        **kwargs
    ) -> Tuple[Figure, Axes]:
    ...
    def wavefunction(
        ...
        a_n_theta = bc.backend.exp(1j * bc.backend.outer(n_vec, theta_vec)) / (2 * bc.backend.pi) ** 0.5
        wavefunc_amplitudes = bc.backend.matmul(state_amplitudes, a_n_theta).T
