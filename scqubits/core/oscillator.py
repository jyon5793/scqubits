...
def harm_osc_wavefunction(
    ...
    if bc.backend.__name__ == 'jax':
        result = bc.backend.pbdv_jax(n, bc.backend.sqrt(2.0) * x / l_osc)
    else:
        result = pbdv(n, bc.backend.sqrt(2.0) * x / l_osc)[0]
    return result[0]
...
class Oscillator(base.QuantumSystem, serializers.Serializable):
    ...
    def eigensys(
        ...
        evecs = bc.backend.zeros(shape=(self.truncated_dim, evals_count), dtype=bc.backend.float_)
        if bc.backend.__name__ == "jax":
            evecs = bc.backend.fill_diagonal(evecs, 1.0)
        else:
            bc.backend.fill_diagonal(evecs, 1.0)
    ...
    def n_operator(self) -> bc.backend.ndarray:
        ...
        a = op.annihilation(self.truncated_dim)
        return 1.0j / (self.l_osc * bc.backend.sqrt(2)) * (a.T - a)
