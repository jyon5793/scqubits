...
from scqubits.utils.helper import convert_to_jax_compatible
...
BareLabel = Tuple[bc.backend.int_, ...]
DressedLabel = bc.backend.int_
...
class ParameterSlice:
    ...
    def __init__(
        self,
        param_name: str,
        param_val: bc.backend.float_,
        fixed_params: Dict[str, bc.backend.float_],
...
class ParameterSweepBase(ABC, SpectrumLookupMixin):
    ...
    _evals_count = descriptors.WatchedProperty(bc.backend.int_, "PARAMETERSWEEP_UPDATE")
    _data = descriptors.WatchedProperty(Dict[str, bc.backend.ndarray], "PARAMETERSWEEP_UPDATE")
    ...
    def subsys_evals_count(self, subsys_index:bc.backend.int_) ->bc.backend.int_:
        return self["bare_evals"]["subsys":subsys_index].shape[-1]

    def dressed_evals_count(self) ->bc.backend.int_:
        """Returns number of dressed eigenvalues included in sweep."""
        return self._evals_count

    def get_subsys_index(self, subsys: QuantumSystem) ->bc.backend.int_:
    ...
    @property
    def subsystem_count(self) ->bc.backend.int_:
    ...
    def _process_initial_option(
        self,
        initial: Optional[Union[StateLabel, List[Tuple[bc.backend.int_]]]],
        subsys_list: List[QuantumSystem],
    ...
    def _process_final_option(
        self,
        final: Optional[Union[StateLabel, List[Tuple[bc.backend.int_]]]],
        initial: StateLabel,
        subsys_list: List[QuantumSystem],
        sidebands: bool,
    ...
    def transitions(
        ...
        photon_number:bc.backend.int_ = 1,
    ...
    def transitions(
        ...
        subsystems: Optional[Union[QuantumSystem, List[QuantumSystem]]] = None,
        initial: Optional[Union[StateLabel, List[Tuple[bc.backend.int_]]]] = None,
        final: Optional[Union[StateLabel, List[Tuple[bc.backend.int_]]]] = None,
        sidebands: bool = False,
        photon_number:bc.backend.int_ = 1,
        make_positive: bool = False,
        param_indices: Optional[NpIndices] = None,
    ...
    def transitions(
        ...
        for initial in initial_states:
            ...
            for final in final_states:
                ...
                for final_state in final_states_list:
                    ...
                    diff_energies = (final_energies - initial_energies).astype(bc.backend.float_)
                    diff_energies /= photon_number
                    if make_positive:
                        diff_energies = bc.backend.abs(diff_energies)
                    if not bc.backend.isnan(
                        diff_energies
        ...
        if self._slice_is_1d_sweep(param_indices):
            ...
            return SpectrumData(
                energy_table=bc.backend.asarray(transition_energies).T,
                system_params=self.system_params,
                param_name=name,
                param_vals=vals,
                labels=label_list,
                subtract=bc.backend.asarray(
                    [initial_energies] * self._evals_count, dtype=bc.backend.float_
                ).T,
            )

        return SpectrumData(
            energy_table=bc.backend.asarray(transition_energies),
            system_params=self.system_params,
            labels=label_list,
        )
    ...
    def plot_transitions(
        ...
        coloring: Union[str, bc.backend.ndarray] = "transition",
        param_indices: Optional[NpIndices] = None,
        **kwargs,
    ) -> Tuple[Figure, Axes]:
        """
        ...
        if make_positive:
            specdata_all.energy_table = bc.backend.abs(specdata_all.energy_table)
...
class ParameterSweep(  # type:ignore
    ...
    def __init__(
        ...
        paramvals_by_name: Dict[str, bc.backend.ndarray],
        update_hilbertspace: Callable,
        evals_count:bc.backend.int_ = 20,
    ...
    def faulty_interactionterm_suspected(self) -> bool:
        ...
        for interactionterm in self._hilbertspace.interaction_list:
            if isinstance(interactionterm, (bc.backend.ndarray, Qobj, bc.backend.csc_matrix)):
                return True
            for idx_operator in interactionterm.operator_list:
                if isinstance(idx_operator[1], (bc.backend.ndarray, Qobj, bc.backend.csc_matrix)):
                    return True
        return False
    ...
    def _bare_spectrum_sweep(self) -> Tuple[NamedSlotsNdarray, NamedSlotsNdarray]:
        ...
        for subsys_index, subsystem in enumerate(self.hilbertspace):
            ...
            if (
                ...
                evals = bc.backend.empty_like(bare_esys[..., 0])
                evecs = bc.backend.empty_like(bare_esys[..., 0])
                if bc.backend.__name__ == "numpy":
                    for array_index, esys in bc.backend.ndenumerate(bare_esys[..., 0]):
                        evals[array_index] = esys[0]
                        evecs[array_index] = esys[1]
                elif bc.backend.__name__ == "jax":
                    shape = bare_esys[..., 0].shape
                    indices = itertools.product(*[range(s) for s in shape])
                    for array_index in indices:
                        esys = bare_esys[array_index]
                        evals = evals.at[array_index].set(esys[0])
                        evecs = evecs.at[array_index].set(esys[1])
            else:
                evals = bare_esys[..., 0]
                evecs = bare_esys[..., 1]
            if bc.backend.__name__ == "numpy":
                bare_evals[subsys_index] = NamedSlotsNdarray(
                    bc.backend.asarray(evals.tolist()),
                    self._parameters.paramvals_by_name,
                )
                bare_evecs[subsys_index] = NamedSlotsNdarray(
                    bc.backend.asarray(evecs.tolist()),
                    self._parameters.paramvals_by_name,
                )
            elif bc.backend.__name__ == "jax":
                bare_evals[subsys_index] = NamedSlotsNdarray(
                    bc.backend.asarray(evals),
                    self._parameters.paramvals_by_name,
                )
                bare_evecs[subsys_index] = NamedSlotsNdarray(
                    bc.backend.asarray(evecs),
                    self._parameters.paramvals_by_name,
                )
        ...
        return (
            NamedSlotsNdarray(bare_evals, {"subsys": bc.backend.arange(self.subsystem_count)}),
            NamedSlotsNdarray(bare_evecs, {"subsys": bc.backend.arange(self.subsystem_count)}),
            NamedSlotsNdarray(
                circuit_esys, {"subsys": bc.backend.arange(self.subsystem_count)}
            ),
        )
    ...
    def _update_subsys_compute_esys(
        ...
        paramval_tuple: Tuple[bc.backend.float_],
    ) -> bc.backend.ndarray:
    ...
    def _subsys_bare_spectrum_sweep(self, subsystem) -> bc.backend.ndarray:
        """
        ...
        total_count = bc.backend.prod([len(param_vals) for param_vals in reduced_parameters])
        ...
        for name in fixed_paramnames:
            ...
            bare_eigendata = bc.backend.repeat(bare_eigendata, param_count, axis=index)
    ...
    def _update_and_compute_dressed_esys(
        ...
        update_func: Callable,
        paramindex_tuple: Tuple[bc.backend.int_],
    ) -> bc.backend.ndarray:
        paramval_tuple = self._parameters[paramindex_tuple]
        update_func(self, *paramval_tuple)
        assert self._data is not None
        bare_esys: Dict[int, List[bc.backend.ndarray]] = {
            subsys_index: [
    ...
    def _dressed_spectrum_sweep(
        ...
        """
        target_map = cpu_switch.get_map_method(self._num_cpus)
        total_count = bc.backend.prod(self._parameters.counts)
        ...
        evals = bc.backend.asarray(spectrum_data_ndarray[..., 0].tolist())
    ...
    def _energies_1(self, subsys):
        bare_label = bc.backend.zeros(len(self.hilbertspace))
        bare_label[self.get_subsys_index(subsys)] = 1

        energies_all_l = bc.backend.empty(self._parameters.counts + (subsys.truncated_dim,))
    ...
    def _energies_2(self, subsys1, subsys2):
        bare_label1 = bc.backend.zeros(len(self.hilbertspace))
        bare_label1[self.get_subsys_index(subsys1)] = 1
        bare_label2 = bc.backend.zeros(len(self.hilbertspace))
        bare_label2[self.get_subsys_index(subsys2)] = 1

        energies_all_l1_l2 = bc.backend.empty(
        ...
        for subsys_index1, subsys1 in enumerate(self.hilbertspace):
            ...
            for subsys_index2, subsys2 in enumerate(self.hilbertspace):
                ...
                if subsys1 in self.osc_subsys_list and subsys2 in self.osc_subsys_list:
                    ...
                    chi_data[subsys_index1, subsys_index2] = bc.backend.asarray([])
                ...
                # ac Stark shifts
                    ...
                    kerr_data[subsys_index1, subsys_index2] = bc.backend.asarray([])

        sys_indices = bc.backend.arange(self.subsystem_count)
...
def generator(sweep: "ParameterSweepBase", func: Callable, **kwargs) -> bc.backend.ndarray:
    ...
    **kwargs:
    ...
    total_count = bc.backend.prod(reduced_parameters.counts)

    def func_effective(paramindex_tuple: Tuple[bc.backend.int_], params, **kw) -> Any:
